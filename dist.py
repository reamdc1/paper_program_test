    ...
    for operon in sorted(operon_result_dict.keys()):
        ...
        for pair in itertools.combinations_with_replacement(org_list, 2):
            ...
            if org1 == org2:
                ...
                if org1 in result.keys():
                    result[org1].update({org2:0})
                else:
                    result.update({org1:{org1: 0}})
            else:
                ...
                if org1 in result.keys():
                    result[org1].update({org2: cost})
                else:
                    result.update({org1:{org2: cost}})
                if org2 in result.keys():
                    result[org2].update({org1: cost})
                else:
